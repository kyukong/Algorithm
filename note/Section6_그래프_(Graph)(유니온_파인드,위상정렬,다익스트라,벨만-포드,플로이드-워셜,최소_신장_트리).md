...
- 최단 거리 알고리즘
- 모든 노드에 대해 최단 거리를 구하는 알고리즘 (시작점이 없음)
- 음수 가중치 엣지가 있어도 수행할 수 있음
- 동적 계획법의 원리를 이용해 알고리즘에 접근
- 시간복잡도 (노드수: V) = `O(V3)`
   - 노드의 세제곱이기 때문에 시간복잡도가 높은 편
   - 최단경로 알고리즘 + 적은 노드 수 → 플로이드 워셜 가능한지 판단

### 핵심 이론

- A 노드에서 B 노드까지 최단 경로를 구했을 때 최단 경로 위에 K 노드가 존재한다면 그것을 이루는 부분 경로 또한 최단 경로
   - 1 → 3 → 4 → 2 → 5 와 같은 최단 경로가 존재할 때, 1에서 4까지의 최단 경로는 1 → 3 → 4
- D[S][E] = Math.min(D[S][E], D[S][K] + D[K][E])
1. 리스트 선언하고 초기화하기

   ![](docs/28.png)

   - 주어진 노드 수가 적기 떄문에 인접 배열로 관리 가능
...
3. 점화식으로 리스트 업데이트하기
   ...
   - for 경유지 `k` 에 대해 (1~N)
      - for 출발 노드 `s` 에 대해 (1~N)
         - for 도착 노드 `e` 에 대해 (1~N)
            - D[S][E] = Math.min(D[S][E], D[S][K] + D[K][E])

## 최소 신장 트리

- 모든 노드를 연결할 때 사용된 엣지들의 가중치의 합이 최소로 하는 트리
- MST
   - 크루스칼 알고리즘, 프림 알고리즘
- 사이클이 포함되면 가중치의 합이 최소가 될 수 없으므로 사이클을 포함하지 않음
- N 개의 노드가 있으면 최소 신장 트리를 구성하는 엣지의 개수는 항상 N - 1 개
